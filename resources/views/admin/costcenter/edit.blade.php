            ...
            <div class="row">
                <div class="offset-md-3 col-md-6">
                    <div class="card card-primary">
                        ...
                        @endif
                        {{ Form::model($costCenter, ['method' => 'POST', 'id' => 'edit-form', 'files' => true]) }}
                        {{ Form::hidden('action', 'edit') }}
                        {{ Form::hidden('id', $costCenter['id'], ['id' => 'id']) }}
                        @include('admin.costcenter.formElements',['submitButtonText' => 'Update'])
                        {{ Form::close() }}
                    </div>
                </div>
            </div>
            ...
            $("#edit-form").validate({
                ...
                submitHandler: function(form) {
                    $.ajax({
                        ...
                        url: "{{ route('cost-center.update', 'id') }}",
                        beforeSend: function() {
                            $("#loadingImage").css({
                                "display": "block"
                            });
